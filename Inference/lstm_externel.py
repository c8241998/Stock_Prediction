...
class MyDataset(Dataset):
        ...
        if mode=='train':
            for i,day in enumerate(date):
                if i<window or '2020-05' in day:
                    continue
                sample = self.get_sample(price,X,window,i)
                self.data.append([
                    sample[0],sample[1]
                ])
        else:
    ...
    def get_sample(self,price, X, window,i):
        factors = X[i - window:i, :].astype(np.float32)
        prices = price[i - window:i].astype(np.float32)
        prices = np.expand_dims(prices,axis=-1)
        sample = np.concatenate((prices,factors),axis=-1)
        return sample , price[i].astype(np.float32)
...
class rnn(paddle.nn.Layer):
    def __init__(self,window):
        ...
        self.lstm = paddle.nn.LSTM(5,5,2)
        self.nn = paddle.nn.Sequential(
            paddle.nn.Flatten(),
            paddle.nn.Linear(window*5,1),
            paddle.nn.Sigmoid()
        )
...
def pipeline(window=10,epochs=100):
    ...
    # x=paddle.randn([16,5,4])
    # y=model(x)
    # print(y.shape)

    # loss:1.06e-5  mse:0.0010